...
const AdminTestManager = () => {
    ...
    const handleSendResults = async (contestId) => {
        try {
            setsendMessageLoader(true);
            const response = await axios.post(`${import.meta.env.VITE_TS_URL}/api/v1/otp/result`, {
                contestId
            });

            if (response.status === 200 || response.status === 207) {
                alert("Result link sent to users successfully!");
            } else {
                alert("Some messages may not have been delivered.");
            }
        } catch (error) {
            console.error("Error sending result link:", error);
            alert("Failed to send result link to users.");
        } finally {
